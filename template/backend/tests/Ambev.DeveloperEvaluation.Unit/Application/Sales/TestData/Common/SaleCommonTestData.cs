...
namespace Ambev.DeveloperEvaluation.Unit.Application.Sales.TestData.Common
{
    public static class SaleCommonTestData
    {
        ...
        public static IEnumerable<Sale> GenerateValidSales(int quantity = 3)
        {
            var sales = SaleFakerBase.Generate(quantity);

            foreach (var sale in sales)
            {
                SaleItemFakeBase
                .RuleFor(s => s.SaleId, sale.Id);
                var items = SaleItemFakeBase.Generate(2);
        ...
        public static SaleItem GenerateValidSaleItem(Guid saleId)
        {
            var saleItem = SaleItemFakeBase.Generate();
